...
export function AssignPickersButton({ shiftId, leaderId, csrf }: { shiftId: number; leaderId: number; csrf: string }) {
    ...
    <>
      ...
      {open && (
        <Modal title="הקצאת עובדים" onClose={() => setOpen(false)}>
          <AssignPickersModal shiftId={shiftId} leaderId={leaderId} csrf={csrf} onClose={() => setOpen(false)} />
        </Modal>
      )}
    </>
  );
