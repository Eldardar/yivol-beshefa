...
export async function POST(req: Request) {
  ...
  try {
    ...
    const result = await new AdminService(database).sendOrScheduleNotification(user.id, candidate);
    return NextResponse.json(result, { headers: noStoreHeaders });
  } catch (error) {
    const message = error instanceof z.ZodError ? (error.issues[0]?.message ?? "קלט אינו תקין") : error instanceof Error ? error.message : "לא ניתן לשלוח את ההודעה";
