...
type AssignmentRow = { id: number; date: string; slot: string; leader_id: number; farm: string; address: string; navigation_link: string | null; crop: string; vehicles: string | null };
...
export default async function Assignments() {
  ...
  const rows = db()
    .prepare(
      `SELECT s.id,s.date,s.slot,s.leader_id,f.name farm,f.address,f.navigation_link,se.crop,GROUP_CONCAT(v.name,', ') vehicles
      ...
      <div className="grid">
        {rows.map(x => (
          <article className="card" key={x.id}>
            ...
            <p className="muted inline-icon-text"><MapPinIcon size={16} /><span>{x.crop} · {x.address}</span></p>
