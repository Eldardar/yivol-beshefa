  ...
  return (
    <AppShell user={user}>
      ...
      <form className="card stack" action="/api/actions" method="post">
        ...
        <div className="grid">
          <div className="field"><label htmlFor="report-hours-start">שעת התחלה בפועל</label><input className="input" id="report-hours-start" type="time" name="hoursStart" required defaultValue={hours?.start_time ?? shift.start_time} /></div>
          <div className="field"><label htmlFor="report-hours-end">שעת סיום בפועל</label><input className="input" id="report-hours-end" type="time" name="hoursEnd" required defaultValue={hours?.end_time ?? shift.end_time} /></div>
        </div>
        <UnitLines
          valueName="qty"
          unitName="unit"
