...
type Row={id:number;name:string;email:string;phone:string;role:"ADMIN"|"PICKER";active:number};
...
export default async function Users({searchParams}:{searchParams:Promise<{saved?:string;error?:string}>}){
  ...
  const users=db().prepare("SELECT id,name,email,phone,role,active FROM users ORDER BY active DESC,role,name").all() as Row[];
  return <AppShell user={user}>
    ...
    <div className="grid"><CreatePicker csrf={csrf}/><section className="card"><h2>חשבונות</h2><div className="table-wrap"><table className="table">
      <thead><tr><th>שם</th><th>תפקיד</th><th>דוא״ל</th><th>טלפון</th><th>מצב</th><th>פעולה</th></tr></thead>
      <tbody>{users.map(row=><tr key={row.id}><td>{row.name}</td><td>{row.role==="ADMIN"?"מנהל":"קוטף"}</td><td>{row.email}</td><td>{row.phone}</td><td><span className={`tag ${row.active?"":"bad"}`}>{row.active?"פעיל":"בארכיון"}</span></td><td>{row.id!==user.id&&<ActiveForm csrf={csrf} id={row.id} active={!row.active}/>}</td></tr>)}</tbody>
