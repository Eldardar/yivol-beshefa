...
export default async function Users({ searchParams }: { searchParams: Promise<{ saved?: string; error?: string }> }) {
  ...
  const users = database.prepare("SELECT id,name,email,phone,national_id,notes,role,active,date_of_birth,favorite_fruit,bank_account_holder,bank_number,bank_name,bank_branch_number,bank_branch_name,bank_account_number FROM users ORDER BY id").all() as WorkerRow[];

  const today = jerusalemDate();
  const shiftRows = database.prepare(`
    SELECT sp.user_id user_id,s.id id,s.date date,s.start_time start_time,s.end_time end_time,s.status status,f.name farm,pf.fruit_type crop
