...
export default async function Resources({searchParams}:{searchParams:Promise<{saved?:string;error?:string}>}){
 ...
 return <AppShell user={user}><h1>ניהול חקלאים ועונות</h1>{saved&&<p className="alert">נשמר בהצלחה</p>}{error&&<p className="alert" role="alert">{error}</p>}
 <div className="grid">
  <form className="card stack" action="/api/actions" method="post"><Hidden csrf={csrf} action="farmCreate"/><h2>חקלאי חדש</h2><input className="input" name="name" placeholder="שם החקלאי" required maxLength={150}/><input className="input" name="contact" placeholder="איש קשר" required maxLength={150}/><input className="input" name="phone" placeholder="טלפון" required maxLength={30}/><input className="input" name="address" placeholder="כתובת" required maxLength={300}/><input className="input" name="navigation" type="url" placeholder="קישור ניווט HTTP(S) (אופציונלי)" maxLength={500}/><textarea className="input" name="notes" placeholder="הערות" maxLength={2000}/><button className="btn">הוספת חקלאי</button></form>
