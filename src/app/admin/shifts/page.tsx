...
export default async function Shifts({searchParams}:{searchParams:Promise<{saved?:string;error?:string;warning?:string;edit?:string}>}){
 ...
 for(const g of goalRows){const m=unitsByShift.get(g.shift_id)??new Map();m.set(g.unit,{unit:g.unit,goal:g.goal,produced:0});unitsByShift.set(g.shift_id,m);}
 for(const p of producedRows){const m=unitsByShift.get(p.shift_id)??new Map();const existing=m.get(p.unit);if(existing)existing.produced=p.produced;else m.set(p.unit,{unit:p.unit,goal:0,produced:p.produced});unitsByShift.set(p.shift_id,m);}
 ...
 </div><div className="field"><span>יעד</span><UnitLines valueName="goalQty" unitName="goalUnit" initial={editGoals} addLabel="הוספת יעד נוסף" valueLabel="יעד" unitLabel="יחידת מידה ליעד"/></div><label>הערות<textarea className="input" name="notes" maxLength={4000} defaultValue={edit?.notes}/></label><div className="actions"><button className="btn">{edit?"שמירת שינויים":"יצירת טיוטה"}</button>{edit&&<Link className="btn secondary" href="/admin/shifts">ביטול עריכה</Link>}</div></form>
 <section className="card"><h2>משמרות</h2><div className="table-wrap"><table className="table"><thead><tr><th>תאריך</th><th>חלק יום</th><th>חקלאי וגידול</th><th>מוביל משמרת</th><th>קוטפים</th><th>יעד / בפועל</th><th>מצב</th><th>פעולות</th></tr></thead><tbody>{shifts.map(s=>{const units=[...(unitsByShift.get(s.id)?.values()??[])];return <tr key={s.id}><td>{formatHebrewDate(s.date)}</td><td>{s.slot==="MORNING"?"בוקר":"ערב"}</td><td>{s.farm} · {s.crop}</td><td>{s.leader}</td><td>{s.picker_count}</td><td>{units.length?units.map(u=>`${u.produced} / ${u.goal} ${UNIT_LABEL[u.unit]}`).join(" · "):"—"}</td><td>{statusName(s.status)}</td><td><div className="actions">{["DRAFT","PUBLISHED"].includes(s.status)&&<Link className="btn secondary" href={`/admin/shifts?edit=${s.id}`}>עריכה</Link>}{s.status==="PUBLISHED"&&<Link className="btn secondary" href={`/leader/${s.id}`}>דוח</Link>}{s.status==="DRAFT"&&<Transition csrf={csrf} id={s.id} target="PUBLISHED" label="פרסום"/>}{s.status==="PUBLISHED"&&<><Transition csrf={csrf} id={s.id} target="COMPLETED" label="סיום"/><Transition csrf={csrf} id={s.id} target="DRAFT" label="החזרה לטיוטה"/></>}{!["COMPLETED","CANCELLED"].includes(s.status)&&<Transition csrf={csrf} id={s.id} target="CANCELLED" label="ביטול" danger/>}{s.status==="CANCELLED"&&<Transition csrf={csrf} id={s.id} target="DRAFT" label="שחזור לטיוטה"/>}</div></td></tr>;})}</tbody></table></div></section>
 </AppShell>;
}
