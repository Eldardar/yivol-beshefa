...
export default async function Shifts({searchParams}:{searchParams:Promise<{saved?:string;error?:string;edit?:string}>}){
 ...
 return <AppShell user={user}><h1>ניהול משמרות</h1>{query.saved&&<p className="alert">הפעולה הושלמה</p>}{query.error&&<p className="alert" role="alert">{query.error}</p>}
 <form key={edit?`edit-${edit.id}`:"new"} className="card stack" action="/api/actions" method="post"><h2>{edit?`עריכת משמרת ${edit.id}`:"משמרת חדשה"}</h2><input type="hidden" name="csrf" value={csrf}/><input type="hidden" name="action" value={edit?"shiftUpdate":"shiftCreate"}/>{edit&&<input type="hidden" name="shiftId" value={edit.id}/>}<div className="grid">
  ...
  <label>מוביל<select className="input" name="leaderId" required defaultValue={edit?String(edit.leader_id):undefined}>{pickers.map(p=><option value={p.id} key={p.id}>{p.name}</option>)}</select></label>
