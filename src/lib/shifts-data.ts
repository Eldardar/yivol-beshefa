...
export function getTotalHoursByWorker(database: Database.Database, today: string, range?: { start: string; end: string }): Record<number, number> {
  const rangeClause = range ? "AND s.date >= ? AND s.date < ?" : "";
  const params = range ? [today, range.start, range.end] : [today];
  const rows = database
    .prepare(
    ...
    .all(...params) as Array<{ user_id: number; start_time: string; end_time: string }>;
