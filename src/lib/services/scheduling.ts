...
export class SchedulingService{
 ...
 private validateAssignments(input:ParsedShift,excludeShiftId?:number,allowLeaderConflict?:boolean,previousPickerIds?:Set<number>):string[]{
  ...
  for(const pickerId of input.pickerIds){
   ...
   if(!picker||picker.role!=="PICKER")throw new Error("קוטף אינו פעיל");
   const retained=previousPickerIds?.has(pickerId)??false;
   if(!picker.active&&!retained)throw new Error("קוטף אינו פעיל");
   if(picker.active&&!isPastShift){
   ...
   if(conflict){
    const isLeader=pickerId===input.leaderId;
    if(isLeader&&allowLeaderConflict)warnings.push(`מוביל המשמרת (${picker.name}) כבר משובץ/ת למשמרת אחרת באותן שעות`);
    else if(isLeader)throw new Error("שיבוץ כפול למוביל המשמרת");
    else throw new Error("שיבוץ כפול");
   }
  }
 ...
 validateExistingShift(shiftId:number,allowLeaderConflict?:boolean):string[]{
  ...
  const personalGoalUnits=(this.db.prepare("SELECT unit FROM shift_goal_units WHERE shift_id=?").all(shiftId) as Array<{unit:string}>).map(x=>x.unit);
  const input=shiftSchema.parse({date:row.date,startTime:row.start_time,endTime:row.end_time,plantationFieldId:row.plantation_field_id,pickerIds,leaderId:row.leader_id,vehicleIds,goals,personalGoalUnits,notes:row.notes});
  return this.validateAssignments(input,shiftId,allowLeaderConflict,new Set(pickerIds));
 }
 createShift(actorId:number,raw:ShiftInput,options?:{allowLeaderConflict?:boolean}):Result{
  ...
  const create=this.db.transaction(()=>{const warnings=this.validateAssignments(input,undefined,options?.allowLeaderConflict);const shiftId=Number(this.db.prepare("INSERT INTO shifts(date,start_time,end_time,plantation_field_id,leader_id,notes,created_by) VALUES(?,?,?,?,?,?,?)").run(input.date,input.startTime,input.endTime,input.plantationFieldId,input.leaderId,input.notes,actorId).lastInsertRowid);const addGoal=this.db.prepare("INSERT INTO shift_goals(shift_id,unit,goal,actual) VALUES(?,?,?,?)");for(const g of input.goals)addGoal.run(shiftId,g.unit,g.goal,g.actual===""||g.actual==null?null:g.actual);const addGoalUnit=this.db.prepare("INSERT INTO shift_goal_units(shift_id,unit) VALUES(?,?)");for(const u of input.personalGoalUnits)addGoalUnit.run(shiftId,u);const addPicker=this.db.prepare("INSERT INTO shift_pickers(shift_id,user_id) VALUES(?,?)");for(const id of input.pickerIds)addPicker.run(shiftId,id);const addVehicle=this.db.prepare("INSERT INTO shift_vehicles(shift_id,vehicle_id) VALUES(?,?)");for(const id of input.vehicleIds)addVehicle.run(shiftId,id);this.db.prepare("INSERT INTO audit_events(actor_id,action,entity_type,entity_id) VALUES(?,?,?,?)").run(actorId,"CREATE","SHIFT",shiftId);return {shiftId,warnings};});
  return create.immediate();
 }
 updateShift(actorId:number,shiftId:number,raw:ShiftInput,options?:{allowLeaderConflict?:boolean}):Result{
  ...
  const update=this.db.transaction(()=>{const existing=this.db.prepare("SELECT status,date FROM shifts WHERE id=?").get(shiftId) as {status:string;date:string}|undefined;if(!existing)throw new Error("המשמרת לא נמצאה");if(!["DRAFT","PUBLISHED","COMPLETED"].includes(existing.status))throw new Error("לא ניתן לערוך משמרת שבוטלה");const oldPickers=(this.db.prepare("SELECT user_id FROM shift_pickers WHERE shift_id=?").all(shiftId) as Array<{user_id:number}>).map(x=>x.user_id);const warnings=this.validateAssignments(input,shiftId,options?.allowLeaderConflict,new Set(oldPickers));this.db.prepare("UPDATE shifts SET date=?,start_time=?,end_time=?,plantation_field_id=?,leader_id=?,notes=?,updated_at=CURRENT_TIMESTAMP WHERE id=?").run(input.date,input.startTime,input.endTime,input.plantationFieldId,input.leaderId,input.notes,shiftId);this.db.prepare("DELETE FROM shift_goals WHERE shift_id=?").run(shiftId);const addGoal=this.db.prepare("INSERT INTO shift_goals(shift_id,unit,goal,actual) VALUES(?,?,?,?)");for(const g of input.goals)addGoal.run(shiftId,g.unit,g.goal,g.actual===""||g.actual==null?null:g.actual);this.db.prepare("DELETE FROM shift_goal_units WHERE shift_id=?").run(shiftId);const addGoalUnit=this.db.prepare("INSERT INTO shift_goal_units(shift_id,unit) VALUES(?,?)");for(const u of input.personalGoalUnits)addGoalUnit.run(shiftId,u);this.db.prepare("DELETE FROM shift_pickers WHERE shift_id=?").run(shiftId);const addPicker=this.db.prepare("INSERT INTO shift_pickers(shift_id,user_id) VALUES(?,?)");for(const id of input.pickerIds)addPicker.run(shiftId,id);this.db.prepare("DELETE FROM shift_vehicles WHERE shift_id=?").run(shiftId);const addVehicle=this.db.prepare("INSERT INTO shift_vehicles(shift_id,vehicle_id) VALUES(?,?)");for(const id of input.vehicleIds)addVehicle.run(shiftId,id);for(const id of oldPickers)if(!input.pickerIds.includes(id))this.db.prepare("DELETE FROM quantities WHERE shift_id=? AND user_id=?").run(shiftId,id);if(existing.status==="PUBLISHED"&&existing.date>=jerusalemDate()){const notify=this.db.prepare("INSERT INTO notifications(user_id,title,body) VALUES(?,?,?)");const phoneOf=this.db.prepare("SELECT phone FROM users WHERE id=?");for(const id of new Set([...oldPickers,...input.pickerIds])){const body=input.pickerIds.includes(id)?`משמרת בתאריך ${input.date} עודכנה`:`הוסרת ממשמרת בתאריך ${input.date}`;notify.run(id,"המשמרת עודכנה",body);pushItems.push({userId:id,title:"המשמרת עודכנה",body});const phone=(phoneOf.get(id) as {phone:string}|undefined)?.phone;if(phone)whatsappItems.push({to:phone,title:"המשמרת עודכנה",body});}}this.db.prepare("INSERT INTO audit_events(actor_id,action,entity_type,entity_id,metadata) VALUES(?,?,?,?,?)").run(actorId,"UPDATE","SHIFT",shiftId,JSON.stringify({published:existing.status==="PUBLISHED"}));return {shiftId,warnings};});
