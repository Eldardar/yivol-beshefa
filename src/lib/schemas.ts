...
const digits=(max:number,message:string)=>z.string().trim().transform(value=>value.replace(/[\s-]/g,"")).pipe(z.union([z.literal(""),z.string().regex(new RegExp(`^\\d{1,${max}}$`),message)]));
export const bankDetailsSchema=z.object({accountHolder:z.string().trim().max(100).default(""),bankNumber:digits(3,"מספר הבנק אינו תקין").default(""),bankName:z.string().trim().max(100).default(""),branchNumber:digits(4,"מספר הסניף אינו תקין").default(""),branchName:z.string().trim().max(100).default(""),accountNumber:digits(13,"מספר החשבון אינו תקין").default("")}).strict().superRefine((x,ctx)=>{const required=[x.accountHolder,x.bankNumber,x.branchNumber,x.accountNumber];if(required.some(Boolean)&&!required.every(Boolean))ctx.addIssue({code:z.ZodIssueCode.custom,message:"יש למלא את כל פרטי החשבון: בעל החשבון, בנק, סניף ומספר חשבון"});});
