...
describe("אבטחה", () => {
  ...
  it("דוחה כמויות שליליות", () => { expect(() => shiftSchema.parse({date:"2026-08-10",slot:"MORNING",seasonId:1,pickerIds:[1],leaderId:1,vehicleIds:[],goals:[{unit:"KG",goal:-1}],notes:""})).toThrow(); });
  it("דוחה יעד עם יחידת מידה כפולה", () => { expect(() => shiftSchema.parse({date:"2026-08-10",slot:"MORNING",seasonId:1,pickerIds:[1],leaderId:1,vehicleIds:[],goals:[{unit:"KG",goal:1},{unit:"KG",goal:2}],notes:""})).toThrow(); });
