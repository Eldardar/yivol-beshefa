...
describe("מחזור משמרת ודיווח", () => {
  ...
  it("מוביל המשמרת שומר שעות ופרטים יחד עם הדיווח",()=>{
    const x=setup();const service=new ShiftService(db);db.prepare("UPDATE shifts SET status='PUBLISHED' WHERE id=?").run(x.shift);
    service.saveQuantities(x.p1,x.shift,[{userId:x.p1,quantity:1,unit:"KG"},{userId:x.p2,quantity:2,unit:"KG"}],{startHour:"06:00",endHour:"10:30",teamLeaderDetails:"הכל תקין"});
    expect(db.prepare("SELECT start_hour,end_hour,team_leader_details FROM shifts WHERE id=?").get(x.shift)).toEqual({start_hour:"06:00",end_hour:"10:30",team_leader_details:"הכל תקין"});
  });
});
