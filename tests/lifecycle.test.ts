...
function setup() {
  ...
  const shift = Number(db.prepare("INSERT INTO shifts(date,start_time,end_time,plantation_field_id,leader_id,notes,created_by) VALUES(?,?,?,?,?,?,?)").run("2026-08-10", "06:00", "12:00", field, p1, "", admin).lastInsertRowid);
...
describe("מחזור משמרת ודיווח", () => {
  ...
  it("מוביל המשמרת שומר שעות ופרטים יחד עם הדיווח",()=>{
    const x=setup();const service=new ShiftService(db);db.prepare("UPDATE shifts SET status='PUBLISHED' WHERE id=?").run(x.shift);
    service.saveQuantities(x.p1,x.shift,[{userId:x.p1,quantity:1,unit:"KG"},{userId:x.p2,quantity:2,unit:"KG"}],{teamLeaderDetails:"הכל תקין"},[{userId:x.p1,startTime:"06:00",endTime:"10:30"}]);
    expect(db.prepare("SELECT team_leader_details FROM shifts WHERE id=?").get(x.shift)).toEqual({team_leader_details:"הכל תקין"});
    expect(db.prepare("SELECT start_time,end_time FROM shift_hours WHERE shift_id=? AND user_id=?").get(x.shift,x.p1)).toEqual({start_time:"06:00",end_time:"10:30"});
  });
  it("מוביל המשמרת יכול לעדכן שעות של קוטף שכבר דווחו",()=>{
    const x=setup();const service=new ShiftService(db);db.prepare("UPDATE shifts SET status='PUBLISHED' WHERE id=?").run(x.shift);
    service.saveQuantities(x.p1,x.shift,[{userId:x.p1,quantity:1,unit:"KG"},{userId:x.p2,quantity:2,unit:"KG"}],undefined,[{userId:x.p2,startTime:"06:15",endTime:"10:45"}]);
    expect(db.prepare("SELECT start_time,end_time,updated_by FROM shift_hours WHERE shift_id=? AND user_id=?").get(x.shift,x.p2)).toEqual({start_time:"06:15",end_time:"10:45",updated_by:x.p1});
    service.saveQuantities(x.p1,x.shift,[{userId:x.p1,quantity:1,unit:"KG"},{userId:x.p2,quantity:2,unit:"KG"}],undefined,[{userId:x.p2,startTime:"06:00",endTime:"11:00"}]);
