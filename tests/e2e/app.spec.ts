...
test("מנהל יוצר, עורך ומפרסם משמרת",async({page},testInfo)=>{
 ...
 await createDialog.getByLabel("תאריך").fill(shiftDate);await createDialog.getByLabel("שעת התחלה").fill(startTime);await createDialog.getByLabel("שעת סיום משוערת").fill(endTime);await createDialog.getByLabel("עונה").selectOption({label:"משק בדיקה · תפוחים"});await createDialog.getByLabel("מוביל משמרת").selectOption({label:"קוטף בדיקה"});await createDialog.getByLabel("יעד").fill("12");await createDialog.getByRole("button",{name:"יצירת טיוטה"}).click();
 const row=page.getByRole("row").filter({hasText:"משק בדיקה · תפוחים"}).filter({hasText:"0 / 12"}).filter({hasText:timeText});await expect(row).toContainText("טיוטה");
 ...
 const editDialog=page.getByRole("dialog",{name:/עריכת משמרת/});await expect(editDialog).toBeVisible();await editDialog.getByLabel("יעד").fill("15");const invalid=await editDialog.locator(":invalid").evaluateAll(nodes=>nodes.map(node=>(node as HTMLInputElement).name));expect(invalid).toEqual([]);await editDialog.getByRole("button",{name:"שמירת שינויים"}).click();await expect(page).toHaveURL(/saved=1/);const updated=page.getByRole("row").filter({hasText:"משק בדיקה · תפוחים"}).filter({hasText:"0 / 15"}).filter({hasText:timeText});await expect(updated).toContainText("0 / 15");
 await updated.getByRole("button",{name:"פרסום"}).click();const published=page.getByRole("row").filter({hasText:"משק בדיקה · תפוחים"}).filter({hasText:"0 / 15"}).filter({hasText:timeText});await expect(published).toContainText("פורסמה");await published.getByRole("link",{name:"דוח"}).click();await page.getByLabel("שעת התחלה").fill("06:00");await page.getByLabel("שעת סיום").fill("10:00");await page.locator('input[name^="qty_"]').fill("7");await page.getByRole("button",{name:"שמירת הדיווח"}).click();await expect(page).toHaveURL(/saved=1/);await expect(page.getByRole("row").filter({hasText:"משק בדיקה · תפוחים"}).filter({hasText:"7 / 15"}).filter({hasText:timeText})).toContainText("7 / 15");
});
