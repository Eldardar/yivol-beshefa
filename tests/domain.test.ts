...
describe("כללי שיבוץ", () => {
  it("דורש מוביל יחיד מתוך הקוטפים", () => {
    ...
    expect(() => service.createShift(x.admin,{date:"2099-08-10",startTime:"06:00",endTime:"12:00",plantationFieldId:x.field,pickerIds:[x.p1,x.p2],leaderId:999,vehicleIds:[x.vehicle],goals:[{unit:"KG",goal:0}],notes:""})).toThrow("המוביל חייב להיות חלק מהצוות");
  });
  it("חוסם חסר זמינות ומאפשר אולי עם אזהרה", () => {
    const x=setup(); const service=new SchedulingService(db);
    db.prepare("DELETE FROM availability WHERE user_id=?").run(x.p2);
    expect(() => service.createShift(x.admin,{date:"2099-08-10",startTime:"06:00",endTime:"12:00",plantationFieldId:x.field,pickerIds:[x.p1,x.p2],leaderId:x.p1,vehicleIds:[],goals:[{unit:"KG",goal:1}],notes:""})).toThrow("אינו זמין");
    db.prepare("INSERT INTO availability(user_id,date,status) VALUES(?,?,?)").run(x.p2,"2099-08-10","MAYBE");
    expect(service.createShift(x.admin,{date:"2099-08-10",startTime:"06:00",endTime:"12:00",plantationFieldId:x.field,pickerIds:[x.p1,x.p2],leaderId:x.p1,vehicleIds:[],goals:[{unit:"KG",goal:1}],notes:""}).warnings).toHaveLength(1);
  });
  it("מאפשר שיבוץ ללא זמינות במשמרת שכבר עברה",()=>{
    const x=setup(); const service=new SchedulingService(db);
    db.prepare("DELETE FROM availability WHERE user_id=?").run(x.p2);
    expect(service.createShift(x.admin,{date:"2000-01-01",startTime:"06:00",endTime:"12:00",plantationFieldId:x.field,pickerIds:[x.p1,x.p2],leaderId:x.p2,vehicleIds:[],goals:[{unit:"KG",goal:1}],notes:""}).warnings).toHaveLength(0);
  });
  it("חוסם שיבוץ במשמרת של חלקת גידול שאינה פעילה",()=>{const x=setup();db.prepare("UPDATE plantation_fields SET active=0 WHERE id=?").run(x.field);expect(()=>new SchedulingService(db).createShift(x.admin,{date:"2099-08-10",startTime:"06:00",endTime:"12:00",plantationFieldId:x.field,pickerIds:[x.p1],leaderId:x.p1,vehicleIds:[],goals:[{unit:"KG",goal:1}],notes:""})).toThrow("חלקת הגידול אינה פעילה");});
