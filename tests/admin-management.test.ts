...
describe("ניהול וארכוב", () => {
  ...
  it("מונע ארכוב משתמש או משאב המשובצים במשמרת תפעולית",()=>{const x=setup();const shift=Number(db.prepare("INSERT INTO shifts(date,slot,season_id,leader_id,created_by) VALUES(?,?,?,?,?)").run("2026-08-10","MORNING",x.season,x.p1,x.admin).lastInsertRowid);db.prepare("INSERT INTO shift_goals(shift_id,unit,goal) VALUES(?,?,?)").run(shift,"KG",1);db.prepare("INSERT INTO shift_pickers(shift_id,user_id) VALUES(?,?),(?,?)").run(shift,x.p1,shift,x.p2);db.prepare("INSERT INTO shift_vehicles(shift_id,vehicle_id) VALUES(?,?)").run(shift,x.vehicle);const service=new AdminService(db);for(const [entity,id] of [["USER",x.p1],["FARM",x.farm],["SEASON",x.season],["VEHICLE",x.vehicle]] as const)expect(()=>service.setActive(x.admin,entity,id,false)).toThrow("משמרת פעילה");});
